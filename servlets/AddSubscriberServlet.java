...
/**
 * Servlet that insert a new subscriber to the DB
 */
...
public class AddSubscriberServlet extends HttpServlet {
	 ...
	 *      response)
	 * @see servlet that insert a subscriber to the subscribers table
	 * @return json format string message indicate whether the adding success or not
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String description = joUser.get("publicChannelDesc").getAsString();
		int numberOfSub = joUser.get("publicChannelNumberOfSub").getAsInt();
		...
		try {
			...
			try {
				stmt = conn.prepareStatement(AppConstants.INSERT_SUBSCRIBER_STMT);

				stmt.setString(1, channelName);
				stmt.setString(2, nickname);
				stmt.setString(3, description);
				stmt.setInt(4, numberOfSub);
				stmt.setInt(5, 0);
				stmt.setInt(6, 0);
